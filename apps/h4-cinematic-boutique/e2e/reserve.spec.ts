...
test.describe("Reserve flow", () => {
  test("shows validation errors when submitted empty", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /request to reserve/i }).click();
    ...
    await expect(page.getByRole("button", { name: /request to reserve/i })).toBeEnabled();
  });
  ...
  test("submits successfully and shows the success state", async ({ page }) => {
    ...
    // Submit — the demo onSubmit resolves after ~600ms.
    await page.getByRole("button", { name: /request to reserve/i }).click();
